...
public class JMCh24Exercises
{
     ...
     * n = birthdays.length.
     * @param birthdays aray of birthdays
     * @return if three birthdays fall on the same day
     */
    @SuppressWarnings("deprecation")
    public boolean has3OnSameDay( Date[] birthdays )
    {
        Map<Date, Integer> countDays = new HashMap<Date, Integer>();
        for ( Date b : birthdays )
        {
            if ( countDays.containsKey( b ) )
            {
                countDays.put( b, countDays.get( b ) + 1 );
                if ( countDays.get( b ) == 3 )
                {
                    return true;
                }
            }
