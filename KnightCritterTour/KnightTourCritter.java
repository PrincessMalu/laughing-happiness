...
public class KnightTourCritter extends KnightCritter
{
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        int nMove = accessGrid[bestMove.getRow()][bestMove.getCol()];

        for ( int i = 0; i < locs.size(); i++ )
        {

            if ( accessGrid[locs.get( i ).getRow()][locs.get( i )
                .getCol()] < nMove )
            {
                bestMove = locs.get( i );
            }
    ...
    public void updateAccess( Location loc )
    {
        ...
        ArrayList<Location> moves = getPossibleMoves( loc );

        for ( int i = 0; i < moves.size(); i++ )
        {
            int row = moves.get( i ).getRow();
            int column = moves.get( i ).getCol();

            accessGrid[row][column]--;
        }
